...
describe('demo telephony', () => {
  it('lets a demo visitor add a gate user without any real telephony', async () => {
    ...
    await createAddGateUser({ directory: createDemoGateUsersDirectory(), gateUsers: repository })({
      name: 'Visitor',
  ...
  it('has nothing to download: no contacts and no calls', async () => {
    ...
    expect(await createSyncGateUsers({ directory: createDemoGateUsersDirectory(), gateUsers: repository })())
