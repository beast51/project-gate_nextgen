...
describe('filterGateUsers', () => {
  ...
  it('searches by one field when a mode is chosen', () => {
    // "12" is in a phone number and in a car number too, and it is the number of one apartment
    expect(filterGateUsers(users, '12', 'apartment').map(u => u.name)).toEqual(['Ivan']);
    expect(filterGateUsers(users, '1', 'apartment')).toEqual([]);
